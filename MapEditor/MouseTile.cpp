...
MouseTile::MouseTile(
	...
	: m_grid(grid), m_tileSize(tileSize), m_tileScale(tileScale), m_offset(offset), m_isMouseOnGrid(false)
{
}
...
void MouseTile::Update(double deltaTime, const sf::Vector2f& mousePosition)
{
	...
	if (mousePosition.x >= gridPosition.x && mousePosition.x < gridPosition.x + gridSize.x
		...
		m_tileGridPosition.x = (mousePosition.x - m_offset.x) / (m_tileSize.x * m_tile.getScale().x);
...
bool MouseTile::IsMouseClickedOnTile(sf::Vector2f& tilePosition,
